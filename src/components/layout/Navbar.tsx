      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <Link to="/" className="flex items-center gap-3">
            <Package className="w-8 h-8 text-[#072C2C]" />
            <div className="flex flex-col">
              <span className="text-lg font-bold text-[#0F172A] leading-tight">SCHub.vn</span>
          ...
          <Link
            to="/checklist"
            className="hidden md:inline-flex px-4 py-2 bg-[#072C2C] text-white text-sm font-medium rounded-lg hover:bg-[#0a4242] transition-colors"
          >
            ...
            {navLinks.map(link => (
              <Link
                ...
                className={`block py-2 text-sm font-medium ${
                  location.pathname.startsWith(link.to) ? "text-[#072C2C]" : "text-slate-600"
                }`}
              >
                {link.label}
              </Link>
            ))}
            <Link to="/checklist" onClick={() => setMobileMenuOpen(false)} className="block py-2 text-sm font-medium text-[#072C2C]">
              Tải checklist 50 điểm
            </Link>
          </div>
        </div>
